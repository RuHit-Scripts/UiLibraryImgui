local SZ
local function _setScale()
	local U = game:GetService("UserInputService")
	local mob = (U.TouchEnabled and not U.KeyboardEnabled) or (U.TouchEnabled and U.MouseEnabled == false)
	SZ = function(v) return math.floor(v * (mob and 1.6 or 1)) end
end
_setScale()
...
tab.ScrollBarThickness = SZ(6)
tab.ScrollBarImageColor3 = Color3.fromRGB(90, 95, 105)
...
label.Size = UDim2.new(0, SZ(200), 0, SZ(24))
label.Font = Enum.Font.GothamSemibold
label.Text = "Hello, world 123"
label.TextColor3 = Color3.new(1, 1, 1)
label.TextSize = SZ(14)
...
resizer.Position = UDim2.new(1, -SZ(30), 1, -SZ(30))
resizer.Size = UDim2.new(0, SZ(30), 0, SZ(30))
resizer.ZIndex = 20
do
	local grip1 = Instance.new("Frame")
	grip1.Name = "GripA"
	grip1.Parent = resizer
	grip1.AnchorPoint = Vector2.new(1, 1)
	grip1.Position = UDim2.new(1, -SZ(3), 1, 0)
	grip1.Size = UDim2.new(0, SZ(12), 0, SZ(2))
	grip1.BackgroundColor3 = Color3.fromRGB(150, 160, 180)
	grip1.BorderSizePixel = 0
	grip1.ZIndex = 21
	local grip2 = Instance.new("Frame")
	grip2.Name = "GripB"
	grip2.Parent = resizer
	grip2.AnchorPoint = Vector2.new(1, 1)
	grip2.Position = UDim2.new(1, 0, 1, -SZ(3))
	grip2.Size = UDim2.new(0, SZ(2), 0, SZ(12))
	grip2.BackgroundColor3 = Color3.fromRGB(150, 160, 180)
...
titleLabel.Size = UDim2.new(0, SZ(200), 0, SZ(24))
titleLabel.Font = Enum.Font.GothamBold
titleLabel.Text = "Gamer Time"
titleLabel.TextColor3 = Color3.new(1, 1, 1)
titleLabel.TextSize = SZ(14)
...
textBox.Size = UDim2.new(1, 0, 0, SZ(24))
textBox.ZIndex = 2
...
slider.BackgroundColor3 = Color3.new(0.145098, 0.14902, 0.156863)
...
sliderValue.Size = UDim2.new(0, SZ(50), 0, SZ(26))
sliderValue.Font = Enum.Font.GothamBold
sliderValue.Text = "0%"
sliderValue.TextColor3 = Color3.new(0.784314, 0.784314, 0.784314)
sliderValue.TextSize = SZ(14)
...
uiListLayout3.Padding = UDim.new(0, SZ(20))
...
button.Size = UDim2.new(1, 0, 0, SZ(26))
button.ZIndex = 2
button.Font = Enum.Font.GothamSemibold
button.Text = "      Folder"
button.TextColor3 = Color3.new(1, 1, 1)
button.TextSize = SZ(14)
...
objects2.Position = UDim2.new(0, SZ(10), 0, SZ(25))
...
horizontalAlignment.Size = UDim2.new(1, 0, 0, SZ(24))
...
local IS_MOBILE = (UIS.TouchEnabled and not UIS.KeyboardEnabled)
	or (UIS.TouchEnabled and UIS.MouseEnabled == false)
local UI_SCALE = IS_MOBILE and 1.6 or 1
ui_options.scale = UI_SCALE
ui_options.min_size = Vector2.new(
	math.floor(400 * math.min(UI_SCALE, 1.15)),
	math.floor(300 * math.min(UI_SCALE, 1.15))
)
...
local mouse
pcall(function() mouse = p:GetMouse() end)
...
local function gMouse()
	...
	if ml.X > 1 or ml.Y > 1 then
		return Vector2.new(ml.X + 1, ml.Y - 35)
	end
	return Vector2.new(lastTouchPos.X + 1, lastTouchPos.Y - 35)
end
...
function library:AddWindow(title, options)
	...
	-- custom drag: move window only by the top bar (touch + mouse)
	do
		local bar = Window:FindFirstChild("Bar")
		local dragActive, dragStart, dragBase = false, nil, nil
		window_data._dragMoved = false
		if bar then
			...
			bar.InputChanged:Connect(function(input)
				if dragActive and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
					...
					if delta.Magnitude > 8 then window_data._dragMoved = true end
					Window.Position = UDim2.new(
						dragBase.X.Scale,
						dragBase.X.Offset + delta.X,
						dragBase.Y.Scale,
						dragBase.Y.Offset + delta.Y
					)
				end
			end)
	...
	do -- Resize Window (IMGUI corner grip, touch + mouse)
		...
		Resizer.InputChanged:Connect(function(input)
			if not resizing or not options.can_resize then return end
			if input.UserInputType ~= Enum.UserInputType.MouseMovement and input.UserInputType ~= Enum.UserInputType.Touch then return end
			local pos = input.Position
			local x = pos.X - Window.AbsolutePosition.X
			local y = pos.Y - Window.AbsolutePosition.Y
			local w = math.max(options.min_size.X, x)
			local h = math.max(options.min_size.Y, y)
			Resize(Window, {Size = UDim2.fromOffset(w, h)}, options.tween_time)
		end)
	...
	do -- [Open / Close] Window (smart collapse: keep only the blue bar)
		local Bar = Window:FindFirstChild("Bar")
		local open_close = Bar and Bar:FindFirstChild("Toggle")
		local open = true
		local canopen = true

		local oldwindowdata = {}
		local oldy = Window.AbsoluteSize.Y
		local oldTitlePos = nil
		local Title = Window:FindFirstChild("Title")

		local function toggleWindow()
			if window_data._dragMoved then return end
			if canopen then
				canopen = false

				if open then
					-- Close: hide everything, keep only the blue bar
					oldwindowdata = {}
					...
					if open_close then Resize(open_close, {Rotation = 0}, options.tween_time) end
					Resize(Window, {Size = UDim2.new(0, Window.AbsoluteSize.X, 0, SZ(26))}, options.tween_time)

					local Base = Bar and Bar:FindFirstChild("Base")
					...
					if open_close then Resize(open_close, {Rotation = 90}, options.tween_time) end
					Resize(Window, {Size = UDim2.new(0, Window.AbsoluteSize.X, 0, oldy)}, options.tween_time)

					local Base = Bar and Bar:FindFirstChild("Base")
					if Base then Base.Transparency = 0 end
		...
		if open_close then
			open_close.MouseButton1Click:Connect(toggleWindow)
		end
		if Bar then
			Bar.MouseButton1Click:Connect(toggleWindow)
		end
	end
	...
	do -- UI Elements
		...
		do -- Add Tab
			function window_data:AddTab(tab_name)
				...
				new_button.Size = UDim2.new(0, gNameLen(new_button), 0, SZ(24))
				...
				do -- Tab Elements
					...
					function tab_data:AddLabel(label_text) -- [Label]
						...
						label.Size = UDim2.new(0, gNameLen(label), 0, SZ(24))
					...
					function tab_data:AddButton(button_text, callback) -- [Button]
						...
						button.Size = UDim2.new(0, gNameLen(button), 0, SZ(26))
					...
					function tab_data:AddSlider(slider_text, callback, slider_options)
						...
						title.Text = slider_text
 						...
 						do -- Slider Math (mouse + touch)
 							local Held = false
 							local isTouch = false
 							local function sliderUpdate(input)
 								if slider_options.readonly then return end
 								local mouse_location
 								if input and (input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseMovement) then
 									mouse_location = input.Position
 								else
 									mouse_location = gMouse()
 								end
 								local x = (mouse_location.X - slider.AbsolutePosition.X) / slider.AbsoluteSize.X
 								x = math.clamp(x, 0, 1)
 								Resize(indicator, {Size = UDim2.new(x, 0, 1, 0)}, options.tween_time)
 								local maxv = slider_options.max
 								local minv = slider_options.min
 								local diff = maxv - minv
 								local sel_value = math.floor((diff * x) + minv)
 							...
 							slider.InputBegan:Connect(function(inputObject)
 								if inputObject.UserInputType == Enum.UserInputType.MouseButton1 or inputObject.UserInputType == Enum.UserInputType.Touch then
 									Held = true
 									isTouch = (inputObject.UserInputType == Enum.UserInputType.Touch)
 									Window.Draggable = false
 									sliderUpdate(inputObject)
 								end
 							end)
 							slider.InputChanged:Connect(function(inputObject)
 								if Held and (inputObject.UserInputType == Enum.UserInputType.Touch or inputObject.UserInputType == Enum.UserInputType.MouseMovement) then
 									sliderUpdate(inputObject)
 								end
 							end)
 							slider.InputEnded:Connect(function(inputObject)
 								if inputObject.UserInputType == Enum.UserInputType.MouseButton1 or inputObject.UserInputType == Enum.UserInputType.Touch then
 									Held = false
 									Window.Draggable = false
 								end
							...
							function slider_data:Set(new_value)
								...
								local minv = slider_options.min
								local maxv = slider_options.max
								local clamped = math.clamp(new_value, minv, maxv)
								local diff = (maxv - minv)
								local percent = diff > 0 and ((clamped - minv) / diff) or 0

								Resize(indicator, {Size = UDim2.new(percent, 0, 0, SZ(26))}, options.tween_time)
					...
					function tab_data:AddKeybind(keybind_name, callback, keybind_options)
						...
						keybind.Size = UDim2.new(0, gNameLen(title) + SZ(80), 0, SZ(24))
					...
					function tab_data:AddColorPicker(callback)
						...
						do -- Color Picker Math
							...
							do
 							...
 							local Held = false
 							local function paletteUpdate(input)
 								local mouse_location = (input and input.Position) or gMouse()
 								...
 								h = x
 								s = 1 - y
 								Resize(palette_indicator, {Position = UDim2.new(0, x * palette.AbsoluteSize.X - (palette_indicator.AbsoluteSize.X / 2), 0, y * palette.AbsoluteSize.Y - (palette_indicator.AbsoluteSize.Y / 2))}, options.tween_time)
 								update()
 							end
 							local function satUpdate(input)
 								local mouse_location = (input and input.Position) or gMouse()
 							...
 							palette.InputBegan:Connect(function(inputObject)
 								if inputObject.UserInputType == Enum.UserInputType.MouseButton1 or inputObject.UserInputType == Enum.UserInputType.Touch then
 									Held = true
 									Window.Draggable = false
 									paletteUpdate(inputObject)
 								end
 							end)
 							palette.InputChanged:Connect(function(inputObject)
 								if Held and (inputObject.UserInputType == Enum.UserInputType.Touch or inputObject.UserInputType == Enum.UserInputType.MouseMovement) then
 									paletteUpdate(inputObject)
 								end
 							end)
 							saturation.InputBegan:Connect(function(inputObject)
 								if inputObject.UserInputType == Enum.UserInputType.MouseButton1 or inputObject.UserInputType == Enum.UserInputType.Touch then
 									Held = true
 									Window.Draggable = false
 									satUpdate(inputObject)
 								end
 							end)
 							saturation.InputChanged:Connect(function(inputObject)
 								if Held and (inputObject.UserInputType == Enum.UserInputType.Touch or inputObject.UserInputType == Enum.UserInputType.MouseMovement) then
 									satUpdate(inputObject)
 								end
 							end)
 							local function colorEnded(inputObject)
 								if inputObject.UserInputType == Enum.UserInputType.MouseButton1 or inputObject.UserInputType == Enum.UserInputType.Touch then
 									Held = false
 									Window.Draggable = false
 								end
 							end
 							palette.InputEnded:Connect(colorEnded)
 							saturation.InputEnded:Connect(colorEnded)
...
library.Themes = {
	Ocean = {
		main_color = Color3.fromRGB(41, 74, 122),
		bg = Color3.fromRGB(21, 22, 23),
		element = Color3.fromRGB(33, 34, 36),
		text = Color3.fromRGB(255, 255, 255),
		accent = Color3.fromRGB(41, 74, 122),
	},
	Night = {
		main_color = Color3.fromRGB(120, 90, 220),
	...
	Candy = {
		main_color = Color3.fromRGB(255, 120, 180),
		bg = Color3.fromRGB(30, 18, 26),
		element = Color3.fromRGB(46, 28, 40),
		text = Color3.fromRGB(255, 245, 250),
		accent = Color3.fromRGB(255, 120, 180),
	},
	Mint = {
		main_color = Color3.fromRGB(0, 180, 140),
		bg = Color3.fromRGB(14, 22, 20),
		element = Color3.fromRGB(22, 34, 30),
		text = Color3.fromRGB(235, 255, 250),
		accent = Color3.fromRGB(0, 180, 140),
	},
	Light = {
		main_color = Color3.fromRGB(41, 74, 122),
		bg = Color3.fromRGB(235, 235, 238),
...
function library:SetTheme(name)
	...
	ui_options.main_color = t.accent
	if typeof(t.bg) == "Color3" then
		-- recolor every window background on next frame
		task.spawn(function()
			task.wait(0.1)
